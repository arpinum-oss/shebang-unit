...
function _file_runner__run_global_setup_if_exists() {
  database__put "sbu_current_global_setup_has_exited" "${SBU_YES}"
  _file_runner__call_function_if_exists "${SBU_GLOBAL_SETUP_FUNCTION_NAME}"
  local status_code=$?
  database__put "sbu_current_global_setup_has_exited" "${SBU_NO}"
  return ${status_code}
}
...
function _file_runner__check_if_global_setup_has_exited() {
  local has_exited="$(database__get "sbu_current_global_setup_has_exited")"
